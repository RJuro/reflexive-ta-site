...
def reconcile(codes: list[dict]) -> list[dict]:
    ...
    except Exception as e:  # degrade gracefully: keep codes unmerged rather than crash
...
def code_sections(conn: sqlite3.Connection, doc_id: str) -> tuple[list[dict], int]:
    ...
    def code_one(task) -> tuple[list[dict], int]:
        prompt, valid = task
        try:
            data = llm.chat_json(system, prompt)  # only the network call runs in the worker
        except Exception as e:  # one flaky section must not crash the whole document
            print(f"  [warn] section coder call failed ({type(e).__name__}); skipping section",
                  file=sys.stderr)
            return [], 0
        return _parse_codes(data.get("codes", []), valid, doc_id)

    with ThreadPoolExecutor(max_workers=CONCURRENCY) as ex:
        results = list(ex.map(code_one, tasks))  # blind, independent, parallel
    raw_codes = [c for codes, _ in results for c in codes]
    dropped = sum(d for _, d in results)
    return raw_codes, dropped


def code_document(conn: sqlite3.Connection, run_id: str, doc_id: str) -> tuple[list[dict], int]:
    """Code a document: parallel blind section coding, then ONE reconcile call to merge
    duplicate codes within the document. Returns (doc_codebook, n_dropped)."""
    raw_codes, dropped = code_sections(conn, doc_id)
    return reconcile(raw_codes), dropped  # one reconcile call per document
...
def code_sections_panel(conn: sqlite3.Connection, doc_id: str, coders: dict[str, str]) -> dict[str, list[dict]]:
    ...
    def run_one(task):
        ...
        except Exception as e:  # one coder failing on one section must not sink the panel
            print(f"  [warn] panel coder '{name}' failed on a section ({type(e).__name__}); skipping",
                  file=sys.stderr)
            codes = []
        return name, codes
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as ex:
        for name, codes in ex.map(run_one, tasks):
...
def reconcile_project(conn: sqlite3.Connection, run_id: str, doc_codebooks: list[list[dict]]) -> dict:
    ...
    project = reconcile([c for cb in doc_codebooks for c in cb])
...
def add_document(conn: sqlite3.Connection, run_id: str, path: Path) -> tuple[str, dict, list[dict], int]:
    ...
    new_codes, dropped = code_document(conn, run_id, doc_id)
...
def demo(doc_names: list[str] | None = None) -> None:
    ...
    for name in doc_names:
        ...
        ds, dc = code_document(conn, run_id, doc_id)
